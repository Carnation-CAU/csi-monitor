...
- GUI: 채널 1·6·11의 링크 품질 자동 비교 및 최종 채널 선택
...
- 행동 라벨, 준비 시간, 수집 시간과 중간 행동 신호를 선택하는 안내형 수집
- 시작 1회·중간 신호 2회·정상 종료 3회의 구분 알림음
- 매트리스 모의 낙상 선택 시 별도 안전 확인과 manifest 기록
...
GUI에서 선택한 프로필이 현재 작업 대상이다. 선택 상태에서 자동 채널 비교를
하면 선택 채널과 `보정 필요` 상태가 그 프로필에 저장된다. 빈 공간 보정을
완료하면 해당 프로필의 기존 임계값을 새 결과로 교체한다. 행동 수집도 같은
...
있다. 실제 배치를 입력하고 빈 공간 보정을 한 번 완료하면 그때부터 값이 채워진다.
